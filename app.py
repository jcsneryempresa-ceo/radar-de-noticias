...
from google import genai as genai_v2 # SDK para compatibilidade se necessário
...
def gemini_generate(prompt: str, temperature: float = 0.7, max_output_tokens: int = 900) -> str:
    ...
    # Configuração do novo SDK genai v2
...
with tabs[0]:
    ...
    assunto_livre = st.text_input("Assunto", placeholder="Ex: picolés, dengue, emprego… (deixe livre se usar Editorial)")
...
with tabs[1]:
    ...
    with col3:
        hour_post = st.time_input("Hora", value=dt.time(18, 0))
