...
import feedparser
import pandas as pd
import google.generativeai as genai  # ✅ Importação padrão
from google import genai as genai_v2 # ✅ SDK novo
...
def gemini_generate(prompt: str, temperature: float = 0.7, max_output_tokens: int = 900) -> str:
    ...
    # Usando o SDK v2 para compatibilidade estável
    ...
    # ✅ Identação corrigida para o bloco de retorno
...
with tabs[0]:
    ...
    assunto_livre = st.text_input("Assunto", placeholder="Ex: picolés, dengue...")
    ...
    if st.button("Buscar notícias"):
        ...
        if df.empty:
            ...
            df["data_ord"] = df["data_ord"].fillna(pd.Timestamp
