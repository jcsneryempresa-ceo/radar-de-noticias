...
import feedparser
import pandas as pd
from google import genai  # ✅ SDK novo: google-genai
...
def gemini_generate(prompt: str, temperature: float = 0.7, max_output_tokens: int = 900) -> str:
    ...
    if not texto:
        raise RuntimeError("A IA não retornou texto.") # ✅ Identação corrigida
    return texto # ✅ Identação corrigida
...
# ✅ Containers definidos globalmente para evitar NameError nas abas
...
with tabs[0]:
    ...
    with colB:
        ...
        # palavras_box e assuntos_box já definidos acima
    ...
    assunto_livre = st.text_input("Assunto", placeholder="Ex: picolés, dengue, emprego…")
    ...
    if st.button("Buscar notícias"):
        ...
        if editorial != "livre":
            ...
            assunto_busca = assunto_livre.strip() or "notícias"
        ...
        for entry in feed.entries[:itens_por_fonte]:
            ...
            resultados.append({
                ...
                "data_txt": dt_item.strftime("%d %b %Y • %H:%M (BRT)") if dt_item else "data não informada",
                "data": dt_item,
                "fonte": "Google News",
                "titulo": titulo,
                "link": link,
